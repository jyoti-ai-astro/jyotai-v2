...
export function PaymentButton({ name, dob, query, email }: PaymentButtonProps) {
  ...
  const onPaymentSuccess = async (paymentResponse: RazorpayResponse, aiPrediction: string) => {
    try {
      ...
      alert("Prediction saved, but email failed. Contact support if needed.");
    }
  };
  ...
  const getPrediction = async (paymentResponse: RazorpayResponse) => {
    setIsLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_AI_BRAIN_API_URL}/api/predict`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ question: query, name, dob: format(new Date(dob), "yyyy-MM-dd") }),
      });
      ...
      await onPaymentSuccess(paymentResponse, aiPrediction);
    } catch (error) {
